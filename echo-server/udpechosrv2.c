// Ref: http://blog.yslin.tw/2011/05/libevent-with-udp.html
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
void read_cb(int fd, short event, void *arg)
{
    ...
    {
        // printf("Read: %s len [%d] - content [%s]\n", inet_ntoa(client_addr.sin_addr), len, buf);

        /* Echo */
        // client_addr.sin_addr.s_addr = inet_addr("140.113.216.33");
        // client_addr.sin_port = htons(80);
        if (sendto(fd, buf, len, 0, (struct sockaddr *)&client_addr, size) == -1)
        {
            perror("sendto()");
        }
    }
